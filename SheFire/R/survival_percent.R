...
#' @seealso \code{\link{shefire}}, \code{\link{survival_depth}}
...
#' 
#' ChamaecristaSeeds2cm <- survival_percent(x = 2, deltaS = 112, deltaH = 147741, model = model_example) #default temporal resolution and temp. Chamaecrista (cassia) seed survival, for parameters see Martin et al (1969)
#' Proteins1.79cm <- survival_percent(x = 1.79, deltaS = 482, deltaH = 248410, res = 0.5, model = model_example) #default temp. Protein thermal denaturation parameters, See Dickinson and Johnson (2004) and Rosenberg et al (1971)
#' 
#' #plot survival
...
survival_percent <- function(x, deltaS, deltaH, res = 1, temp = 48, model){
