...
#' @param overrideClip This parameter will prevent the function from clipping the data set. Default is False. Only change this if you have manually clipped the input data to exactly how you want it analyzed.
#' @param movingWindow This option smooths the data when locating the starting point for heating in the shallow sensor. This can be useful in low temperature fires in particular, when the fire heating may not be faster than diurnal heating on a point to point basis. The default is False.
#' @param windowSize The size (in time points) of the window that is used in smoothing with movingWindow. Default is 3.
...
#' @param corrThreshold Threshold for accepted level of correlation between the input data and the fitted BFD equations as well as between the calculated regression equations and the BFD parameters from the three sensor depths. Correlations less than the threshold will cause the function to throw an error message. Default is 0.8.
#' @param timeBuffer The time (min) added before the temperature begins to rise for the shallow sensor. If you do need extrapolations to depths shallower than your shallow sensor or if you are not calculating the regressions, you can set timeBuffer to 0. The buffer is needed to be able to extrapolate to shallower depths with the parameter regressions. You want to keep the timeBuffer as short as possible because longer buffers can worsen the fits for equations and regressions. The default is set to 30 minutes.
#' @param printPlotsTables Print the standard plots of data (plotted at the temporal resolution of the input data, not "res") and equations as well as the two summary tables containing equation details and fit information. Default is False.
#' @param savePlotsTables Save the standard plots of data (plotted at the temporal resolution of the input data, not "res") and equations as jpeg images and the summary tables containing equation details and fit information as CSV files. Default is False.
...
SheFire <- function(input, sensorDepths = c(5,10, 15), cutOff = 1440, overrideClip = F, movingWindow = F, windowSize = 3, 
                    regression = T, res = 1, corrThreshold = 0.80, timeBuffer = 30, printPlotsTables = F, savePlotsTables = F, saveName = "SheFire", 
                    saveDirectory = getwd()){
  ...
  if(length(sensorDepths) != 3 | mode(sensorDepths) != "numeric"){
  ...
  if(overrideClip != T & overrideClip != F){
    stop("overrideClip must be a boolean T or F")
  }
  if(movingWindow != T & movingWindow != F){
    stop("movingWindow must be a boolean T or F")
  }
  if(length(windowSize) != 1 | mode(windowSize) != "numeric"){
    stop("windowSize must be a single number")
  }
  ...
  if(savePlotsTables != T & savePlotsTables != F){
    stop("savePlotsTables must be a boolean T or F")
  }
  #packages
  if(movingWindow == T){
    require(evobiR)
  ...
  if(savePlotsTables == T){ #set working directory for saving
  ...
  if (movingWindow == T & overrideClip == F) {
  ...
  if(movingWindow == F & overrideClip == F) {
    ...
    if(StartInd <= 0){  #if timeBuffer sets StartInd to be negative
      ...
      print("Warning: StartInd was set to the first time point because the timeBuffer was set too large for this input data (it would have set the StartInd prior to the start of the data)")
    }
  } 
  #Setting EndInd
  if (overrideClip == F) {
  ...
  if(savePlotsTables == T){
    ...
    write.csv(summary.table.df, paste0(saveName, "_summarytable.csv"))
  }
  #Time info
  ...
  if(InitTemp.pearson$estimate < corrThreshold){ #if InitTemp pearson correlation is below threshold, use closest sensor for InitTemp
    InitTemp.reg = function(x, InitTemp.coeffs, InitTemp.byDepth, SensorDepths) { #same parameters as previous function to standardize function calls
  ...
  if(savePlotsTables == T){
    write.csv(regressionSummary.df, paste0(saveName, "_regtable.csv"))
  ...
  model.list <- list("BFDEquation" = BFDEquation, "MaxTemp.reg" = MaxTemp.reg, "MaxTemp.coeffs" = MaxTemp.coeffs, "TimeAtMax.reg" = TimeAtMax.reg, "TimeAtMax.coeffs" = TimeAtMax.coeffs, 
                     ...
                     "SensorDepths" = sensorDepths, "Shallowest" = Shallowest, "FullTime" = FullTime, "StartTime" = StartTime, "EndTime" = EndTime)
  return(model.list)
}
