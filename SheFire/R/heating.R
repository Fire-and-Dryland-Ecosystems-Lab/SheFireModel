...
#' @author MaryKBrady
#' @seealso \code{\link{shefire}}, \code{\link{cooling}}
...
#' 
#' heating_portion <- heating(x = 4, model = model_example)  #res defaults to 1 minute
#' @export

heating <- function(x, res = 1, model){
  #x and res checked in temp_over_time function
  equation <- temp_over_time(x = x, res = res, model = model)  #function defined in package
