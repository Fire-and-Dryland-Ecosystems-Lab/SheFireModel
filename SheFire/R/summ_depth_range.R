...
#' 
#' Depth5to10cm <- summ_depth_range(x = c(5, 10), inc = 1, model = model_example) #t defaults to FullTime - full time range of model, res defaults to 1
#' 
...
summ_depth_range <- function(x, inc = 0.5, t = NULL, res = 1, model){
  #x and t  and res checked in temp_over_time function
  if(is.null(t)){
    t <- model$FullTime
  }
  ...
  allTemps <- sapply(depths, FUN = temp_over_time, t = t, res = res, model = model) #create equation for each depth
