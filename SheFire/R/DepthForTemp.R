...
#' 
#' Deepest30 <- DepthForTemp(temp = 30, inc = 0.1, model = model_example)  #default temporal resolution res = 1
#' @export

DepthForTemp <- function(temp, inc = 0.5, res = 1, model){
  ...
  firstcheck.table <- sapply(depths, FUN = TempOverTime, res = res, model = model) #get temps for each depth, t defaults to FullTime
  ...
  secondcheck.table <- sapply(depths, FUN = TempOverTime, res = res, model = model)  #get temps for each depth
  ...
  thirdcheck.table <- sapply(depths, FUN = TempOverTime, res = res, model = model)  #get temps for each depth
