...
#' @author MaryKBrady
#' @seealso \code{\link{shefire}}, \code{\link{heating}}
...
#' model_example <- SheFshefireire(input = inputFile )  #create model with all default settings
#' 
#' CoolingDown <- cooking(x = 5, res = 1, model = model_example)
#' @export

cooling <- function(x, res = 1, model){  #is x a valid depth
  #x and res checked in TempOverTime function
  equation <- temp_over_time(x = x, res = res, model = model) #t defaults to FullTime
