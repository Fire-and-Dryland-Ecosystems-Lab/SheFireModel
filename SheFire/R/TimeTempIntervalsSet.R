#' Time in Temperature Intervals Set
#' 
...
#' @author MaryKBrady
#' @seealso \code{\link{SheFire}}, \code{\link{TimeTempIntervals}}
...
#' 
#' SetBins2cm <- TimeTempIntervalsSet(x = 2, bins = c(10,15,20,25,30,35,40), model = model_example) #default res is 1 min
#' 
#' #example to compare diff depths with same temp bins - reformat table output from TempIntervalset function to plot
#' CompareDepths <- TimeTempIntervalsSet(x = c(2,3,4,5), bins = seq(from = 10, to = 45, by = 5), model = model_example)
...
TimeTempIntervalsSet <- function(x, bins=c(0, 10, 20, 30, 40, 50, 60), res = 1, model){
  #x checked in TempOverTime function
  ...
  for (a in seq(length(x))){
    equation <- TempOverTime(x = x[a], res = res, model = model)
