...
TempOverTime <- function(x, t = NULL, res = 1, model){
  ...
  if (x > (model$SensorDepths[3]+5)){ #warning if trying to use deeper then deepest sensor + 5cm
    print("Warning: No deepest depth that can be predicted by this model has been set. Until someone is able to conduct further validation experiments, use caution predicting more than about 5cm beyond your deepest sensor. Maybe plot the temperature over time for the deep depth along side a few shallower depths and see if the results makes sense")
  }
