...
#'
#' @param survival.value The survival threshold of interest in decimal form (60 percent survival -> 0.6).
...
#' @seealso \code{\link{shefire}}, \code{\link{survival_percent}}
...
#' 
#' ChamaecristaSeeds <- survival_depth(survival.value = 0.8, deltaS = 112, deltaH = 147741, inc = 0.1, model = model_example) #default temporal resolution, increment, and temp. Chamaecrista (cassia) seed survival, for parameters see Martin et al (1969)
#' Proteins <- survival_depth(survival.value = 0.5, deltaS = 482, deltaH = 248410, inc = 0.5, model = model_example) #default temporal resolution and temp. Protein thermal denaturation parameters, See Dickinson and Johnson (2004) and Rosenberg et al (1971)
#' @export

survival_depth <- function(survival.value, deltaS, deltaH, temp = 48, inc = 0.5, res = 1, model){
  #res checked in temp_over_time functions
  if(mode(survival.value)!= "numeric" | length(survival.value) != 1 | survival.value > 1 | survival.value < 0){  #is survival.value valid
    stop("survival.value must be a single number from 0 to 1")
  }
  ...
  for(a in depths){
    ...
    equation <- temp_over_time(x = a, res = res, model = model) #for each time point
  ...
  exact <- which(survival.list == survival.value) #test for exact value
  if(length(exact) != 0){ #if any are exact
    ...
    print(paste("Depth", finaldepth, "cm had a survival of", survival.value))
    return(finaldepth)
  }
  exceeds <- which(survival.list > survival.value) #which depths got over the threshold (survival.list index same as depths index)
  if(length(exceeds) == 0){  #if no depths exceed survival threshold
    deepestsurvival <- tail(survival.list, n=1)
    print(paste("All depths tested had lower survival than", survival.value, ". The model does not test deeper than 5cm beyond the deepest sensor where, in this case, the survival was", model$SensorDepths[3]+5 ,"cm with", round(deepestsurvival, digits = 3), "survival."))
    return(NA)
  }
  ...
  exceeds <- which(survival.list > survival.value)
  if(length(exceeds)==length(survival.list)){ #all could exceed survival if starting from model$Shallowest in depths
    print(paste("No depths tested had a survival at or below", survival.value, ". The shallowest depth that could be tested was", model$Shallowest,"cm which had a survival of", round(survival.list[1], digits = 3)))
    return(NULL)
  }
  ...
  notexceeds <- which(survival.list < survival.value) #which depths are lower survival than threshold
