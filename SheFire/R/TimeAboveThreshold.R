...
#' 
#' TimeAbove30 <- TimeAboveThreshold(x = 5, temp = 30, res = 1, model = model_example) #t defaults to FullTime - full time range of model
#' TimeAbove20 <- TimeAboveThreshold(x = 8, temp = 17, t = 10:1000, res = 0.5, model = model_example) #subset the time period used
#' 
#' #example with multiple depths
#' depths <- c(4, 6, 8)
#' Above25multiple <- sapply(X=depths, FUN = TimeAboveThreshold, temp = 25, t = 1100, model = model_example) #shortened time range
#' @export

TimeAboveThreshold <- function (x, temp, t= NULL, res = 1, model){
  #x and t and res are checked in TempOverTime function
  if (mode(temp) != "numeric" | length(temp) != 1){  #is temp valid input
    stop("The temp threshold must be a single number")
  }
  equation <- TempOverTime(x = x, t = t, res = res, model = model)  #function defined in package
