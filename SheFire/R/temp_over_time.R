...
#' 
#' Depth4cm <- temp_over_time(x = 4, res = 1, model = model_example) #run function on model, t defaults to FullTime - full time range of model
#' 
...
#' depths <- c(1, 2, 3, 4, 5)
#' multiple.depths <- sapply(X=depths, FUN=temp_over_time, model = model_example) #default t and res
...
temp_over_time <- function(x, t = NULL, res = 1, model){
